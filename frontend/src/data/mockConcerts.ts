...
// Fonction utilitaire pour obtenir un concert par ID
export const getConcertById = (concertId: string): Concert | undefined => {
  return mockConcerts.find(concert => concert.concertId === concertId);
};

// Fonction utilitaire pour filtrer les concerts par statut
export const getConcertsByStatus = (status: Concert['status']): Concert[] => {
  return mockConcerts.filter(concert => concert.status === status);
};

// Fonction utilitaire pour obtenir les concerts à venir
export const getUpcomingConcerts = (): Concert[] => {
  return getConcertsByStatus('upcoming');
};

// Fonction utilitaire pour obtenir les concerts en direct
export const getLiveConcerts = (): Concert[] => {
  return getConcertsByStatus('live');
};
